...
const UserSearchBox: React.FC = () => {
  ...
  const [savedSearchSubstring, setSavedSearchSubstring] = useState(user.searchString);
  ...
  const handleSubmitClick = () => {
    if (user.searchString !== savedSearchSubstring) {
  ...
  return (
    <Box>
      ...
      <Input placeholder="Enter username" onChange={handleInputChange} value={user.searchString} />
      <Button onClick={handleSubmitClick}>Search</Button>
      {Array.isArray(user.data) && user.data.length ? (
        <SearchString>Showing users for "{savedSearchSubstring}"</SearchString>
      ) : null}
    </Box>
  );
};
